...
In this section we provide the results and compare a query performance given different ranges in the physical layouts presented in Blinktopus(LogSV, RowSV, ColSV and AQPSV). \\

Blinktopus was implemented in Dropwizard(Java 1.8) for back end side and Jupyter Notebook(Phython 2.7) for front end. The experiments were computed on a commodity multi-core machine running CentOS Linux 7.1.1503 and Java SDK 8u131-b11-linux-x64, with 2 Intel(r) Xeon (TM) E5-2630 v3s CPU @ 3.2 GHz processors (8 cores each) and 1024 GiB of Memory. 

In Blinktopus we exploit TPC-H datasets, namely Orders and LineItem relations, but for our evaluation we decided to run queries only over totalprice attribute from Orders.
...
\begin{figure} \includegraphics[width=0.4\textwidth, center]{img/exp1.jpeg} 
\caption{Experiment 1.}
\end{figure}

In the first experiment we compare the performance of the query over Log, Row and Column storage views and Fig.2 demonstrates its results. 

As we expected in almost all cases the scan over LogSV obtained the worst results, while ColSV performed slightly better than RowSV. However, surprisingly for us, in the last case the query over ColSV was not as fast as usual and together with LogSV demonstrated almost similar performance.  

In the second experiment we repeated the same test as in the first, additionally including the tests over AQPSV. 

For our AQPSV, we have decided to build histograms on the columns of extendedprice from LineItem relation and totalprice from Orders. They were constructed on the whole dataset for COUNT queries. Furthermore, we needed to sort the data of the given attribute. 

As for sketches on DISTINCT COUNT queries, we estimate the cardinality of totalprice attribute from Orders.

The results for the second experiment can be seen from Fig. 3. It illustrates that among all SVs, AQP shows a considerably better performance and has the smallest query runtime. In the second case ColSV again demonstrated unexpectedly worst performance.

\begin{figure} \includegraphics[width=0.4\textwidth, center]{img/exp2.jpeg} 
\caption{Experiment 2.}
\end{figure}

\begin{figure} \includegraphics[width=0.4\textwidth, center]{img/exp3.jpeg} 
