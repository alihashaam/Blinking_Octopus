...
Log is most probably one of the simplest data structures for DBMS. Furthermore, by supporting an appending write and sequential read, logs may not even require additional access methods. Therefore these systems eventually may show a better performance. Samza[31], OctopusDB[1] are examples of the systems that exploit logs as their primary storage. Where Samza also supports a so-called \textit{log compaction mode}, in which older messages are garbage-collected[31]. Moreover, in contrast to OctopusDB and Blinktopus, in Samza these logs are also being replicated(i.e., more scalable).\\
